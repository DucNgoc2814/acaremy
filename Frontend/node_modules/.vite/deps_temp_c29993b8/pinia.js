      ...
      if (payload.app === app && payload.inspectorId === INSPECTOR_ID) {
        ...
        if (inspectedStore) {
          if (payload.nodeId !== PINIA_ROOT_ID)
            globalThis.$store = (0, lib_exports.toRaw)(inspectedStore);
...
function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
  ...
  const _hmrPayload = (0, lib_exports.markRaw)({
    actions: {},
    getters: {},
    state: [],
    hotState
  ...
  const setupStore = runWithContext(() => pinia._e.run(() => (scope = (0, lib_exports.effectScope)()).run(() => setup({ action }))));
...
function defineStore(idOrOptions, setup, setupOptions) {
  ...
  function useStore(pinia, hot) {
    ...
    (false ? null : pinia) || (hasContext ? (0, lib_exports.inject)(piniaSymbol, null) : null);
    if (pinia)
      setActivePinia(pinia);
    if (!activePinia) {
...
function storeToRefs(store) {
  if (isVue2) {
    return (0, lib_exports.toRefs)(store);
  } else {
    const rawStore = (0, lib_exports.toRaw)(store);
    const refs = {};
    for (const key in rawStore) {
      ...
      } else if ((0, lib_exports.isRef)(value) || (0, lib_exports.isReactive)(value)) {
        refs[key] = // ---
        (0, lib_exports.toRef)(store, key);
      }
    }
    return refs;
  }
